...
val metrics = List(
  ("SERVER_HITS", count("*") as "hits"),
  ("SERVER_THROUGHPUT", sum("responseInfo.contentSize") as "throughput"),
  ("SERVER_DISTINCT_IPS", countDistinct("clientInfo.ipAddress") as "distinct-ips"))

metrics.foreach(m => {
  m match {
    case (metric, function) => {

      val fw = new java.io.FileWriter("oma" + "-2016-" + m)
